...
object Lift : SubsystemBase() {
    ...
    override fun periodic(){
        ...
        if(motor.encoder.position >= constants.liftEncoderMax){
            if(motor.get()>0){
                motor.stopMotor()
            }
        }else if(motor.encoder.position<=0.0){
            if(motor.get()<0){
                motor.stopMotor()
            }
        }
    ...
    fun setMotor(speed:Double){
        if((motor.encoder.position >= constants.liftEncoderMax) && (speed > 0.0)){
            return
        }else if((motor.encoder.position <= 0.0) && (speed < 0.0)){
            return
        }
        motor.set(speed)
    }
